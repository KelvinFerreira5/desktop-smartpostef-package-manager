    ...
    pub fn delete_file(file_path: String) -> Result<bool, String> {
        ...
        if path.exists() {
            std::fs::remove_file(path).map_err(|e| {
                log_to_file("ERROR", "DELETE_FILE: Failed to delete file", Some(&format!("{}: {}", file_path, e)));
                e.to_string()
            })?;
            log_to_file("INFO", "DELETE_FILE: File deleted", Some(&file_path));
            Ok(true)
        } else {
            Ok(false)
        }
